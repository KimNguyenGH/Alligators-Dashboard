...
```{r}
hashtags20 <-
  kbl(head(tstat_freq_twenty, 20, digits = 2), booktabs = T) %>% 
  kable_styling()

hashtags21 <-
  kbl(head(tstat_freq_twentyone, 20, digits = 2), booktabs = T) %>% 
  kable_styling()
...
twenty_rts %>% group_by(ScreenName) %>%
  ...
  left_join(twenty_uqtweets %>% 
              filter(n>1), by = c('ScreenName')) %>% 
  mutate(`rt_index*1e7` = ((Retweets/n)/Followers) * 10000000) %>% 
  arrange(rt_index) %>% head(10) %>% 
