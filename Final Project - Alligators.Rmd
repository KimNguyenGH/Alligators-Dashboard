...
To examine the differences in sentiment between March 30, 2020 and March 31, 2021, we use sentiment analysis through text mining in R and use several methods to visualize and interpret the different aspects of the tweet, including length, content, and source. We use random forest model and backward stepwise regression to predict the virality of the tweets based on selected variables. We chose to use random forest as (1) it works well with both categorical and continuous values, (2) it reduces overfitting in decision trees, leading to a higher accuracy. Backward stepwise regression is chosen as (1) it is faster than other models, (2) it shows us the removed/added variables, providing valuable information about the quality of the predictor variables.
...
Looking at both the 2020 and 2021 sentiments, we can see that the word "pandemic" has beenmost used, but with different frequency. In 2021, other words started to gain in similar popularity as the word "pandemic", with negatively valenced words such as "bad" and "shit", and positively valenced words such as "hope" and "love". This is interesting to note because it demonstrates that after a year, people are exhibiting heightened emotions, most likely from exhaustion in dealing with the pandemic. These sentiments indicate people's expression when it comes to coping with the pandemic.
