...
twenty_afinn_table <- plot_ly(
  ...
  #  fill = list(color = c("#1DA1F2","#1DA1F2")),
    font = list(family = "Arial Black", size = 14.5, face = "bold")
  ),
  cells = list(
  ...
  ))  %>%  layout(title = "30 Mar 2020 AFINN Scores By Word", font = twenty_afinn_title, margin = list(t = -8))
...
twentyone_afinn_table <- plot_ly(
  ...
  ))  %>%  layout(title = "30 Mar 2021 AFINN Scores By Word", font = twentyone_afinn_title, margin = list(t = -8) )
...
Next, we use the quanteda package for topic modeling to examine prevalent topics and discussions. This algorithm determines the clusters of words that are likely to be associated, thus defining the topics. Topic modelling provides us insight as to the different conversations associated with COVID-19 people are having during this period.
...
```{r }
HTML(ggplot_image(twenty_sentiment_analysis3, height = px(650))) 

HTML(ggplot_image(twentyone_sentiment_analysis3, height = px(650))) 

```

***

Looking at both the 2020 and 2021 sentiments, we can see that the words "pandemic" have been the most used word, but with different frequency. In 2021, other words started to gain in similar popularity as the word "pandemic", with negatively valenced words such as "bad" and "shit", and positively valenced words such as "hope" and "love". This is interesting to note because it demonstrates that after a year, people are exhibiting heightened emotions, most likely from exhaustion in dealing with the pandemic. These sentiments indicate people's expression when it comes to coping with the pandemic.
...
In 2020, the word "support" had the most frequency and is positively valenced, whereas in 2021, the word "stop" had the most frequency and is negatively valenced. What is interesting to note is that "support" and "stop" are opposite words. This demonstrate that perhaps initially, there were certain efforts people wanted to advocate and promote to mitigate effects of the pandemic. Over the course of the year, it could be that people grew exhausted of the pandemic and wanted certain aspects to stop.
...
browsable(div(
  ...
  div(ggplotly(cntrymentions21), tags$br(), style = "width: 50%;"),
  div(HTML(ggplot_image(cntryplot20, height = px(600))), style = "width: 50%; justify-content: center;"),
  div(HTML(ggplot_image(cntryplot21, height = px(600))), style = "width: 50%; justify-content: center;"),
  p('---')
))
```
...
browsable(div(
  style = "display: flex; flex-wrap: wrap;",
  h2("2020", style = "width: 100%; font-family: sans-serif;"),
  div(HTML(worstrt20), style = "width: 100%;"),
  h2("2021", style = "width: 100%; font-family: sans-serif;"),
...
ggplot(data = twenty_rts_wordcounts, aes(x = tweetLength)) + geom_histogram(fill="firebrick") + ggtitle("30 Mar 2020 - Histogram of Tweet Length") -> rtlength20

ggplot(data = twentyone_rts_wordcounts, aes(x = tweetLength)) + geom_histogram(fill="firebrick") + ggtitle("30 Mar 2021 - Histogram of Tweet Length") -> rtlength21
